...
def try_s1(problem,s1_solution):
    ...
    if (s1_correctness >= temp_thresholds.correctness_threshold):
...
def metacognition(problem, system1_solver, system2_solver, context_file, thresholds_file):
    ...
    s1_confidence = system1_solver.confidence_estimate(s1_solution)
    ...
    tested_s1 = False
    correctness_s1=0
    if (model_of_self.count_solved_instances(systemALL,plannerALL) > temp_thresholds.threshold1):
        ...
        if (model_of_self.count_solved_instances(systemONE,plannerALL) > temp_thresholds.threshold4):
            M = 1-model_of_self.get_avg_corr(systemONE,plannerALL,temp_thresholds.threshold4)
        else:
            M = 0

        if (s1_confidence * (1-M) > temp_thresholds.correctness_threshold):
    ...
    reamining_time = thresholds_file.time_limit - (time.time() - timerSOFAI)
    ...
    # if System-1 failed we try using System-2 even if we think that we do not have ebough time
        ...
        probability_s1 = (1-temp_thresholds.threshold3)*temp_thresholds.epsilon_s1
        ...
        ## Instead, with 1-epsilon, we use the ''standard'' metacognitive path
            ...
            if (s1_correctness >= correctnessCntx):
                if((1-(estimated_cost_s2 * (1-temp_thresholds.threshold3))) > (s1_correctness*(1-M))):
