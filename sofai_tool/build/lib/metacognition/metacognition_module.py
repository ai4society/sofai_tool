...
def try_s1(problemId,system1_solver,s1_solution,correctness_threshold,s1_confidence,S1_time,timerSOFAI):
    ...
    if not tested_s1:
        ...
        s1_correctness = system1_solver.calculate_correctness(problemId,s1_solution)
        if (s1_correctness >= correctness_threshold):
            model_of_self.memorize_solution(systemONE, problemId, s1_confidence, S1_time, s1_correctness, s1_solution, (time.time() - timerSOFAI), 0)
...
def try_s2(problemId,system2_solver,correctness_threshold,timerSOFAI,time_limit_context,difficulty=-1.0):
    ...
    if not tested_s2:
        ...
        if difficulty < 0:
            system2_solver.estimate_difficulty(problemId)
        
        s2_time = time.time()
        s2_confidence,s2_solution = system2_solver.solve(problemId,time_limit_context - (time.time() - timerSOFAI))
        s2_time = time.time() - s2_time 
        s2_correctness = system2_solver.calculate_correctness(problemId,s2_solution)
        if (s2_correctness >= correctness_threshold):
            model_of_self.memorize_solution(systemTWO,problemId, s2_confidence, s2_time, s2_correctness, s2_solution, (time.time() - timerSOFAI), difficulty)
...
def metacognition(problemId, system1_solver, system2_solver, context_file, thresholds_file):
    ...
    if (model_of_self.count_solved_instances(systemALL) > T1):
        ...
        if (s1_confidence * (1-M) > correctness_threshold):
            s1_correctness = max(s1_correctness,try_s1(problemId,system1_solver,s1_solution,correctness_threshold,s1_confidence,s1_time,timerSOFAI))
    ...
    #print(f"Test S1 corr {system1_solver.calculate_correctness(problemId,s1_solution)} and sol {s1_solution}")
    ...
    if (estimated_cost_s2 > 1):
        s1_correctness = max(s1_correctness,try_s1(problemId,system1_solver,s1_solution,correctness_threshold,s1_confidence,s1_time,timerSOFAI)) #Exiting from function already implied that S1 is not good enough
    ...
    # if System-1 failed we try using System-2 even if we think that we do not have ebough time
        ...
        if (probability_s1 > random.random()):
            s1_correctness = max(s1_correctness,try_s1(problemId,system1_solver,s1_solution,correctness_threshold,s1_confidence,s1_time,timerSOFAI)) #Exiting from function already implied that S1 is not good enough
            s2_correctness = max(s2_correctness,try_s2(problemId,system2_solver,correctness_threshold,timerSOFAI,estimated_difficulty_s2,time_limit_context))
            utilities.end_computation(problemId, timerSOFAI)
        ...
        ## Instead, with 1-epsilon, we use the ''standard'' metacognitive path
            ...
            if (s1_correctness >= correctness_threshold):
                if((1-(estimated_cost_s2 * (1-T3))) > (s1_correctness*(1-M))):
                    ...
                    model_of_self.memorize_solution(systemONE, problemId, confidenceS1, s1_time, s1_correctness, s1_solution, timerSOFAI,estimated_difficulty_s2)
                else:
                    model_of_self.memorize_solution(systemONE, problemId, confidenceS1, s1_time, s1_correctness, s1_solution, timerSOFAI,estimated_difficulty_s2)
                print(f"Solution found by System 1: {s1_solution} with correctness {s1_correctness}")
                sys.exit()
